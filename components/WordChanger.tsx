...
export const WordChanger: React.FC<WordChangerProps> = ({ styles }: WordChangerProps) => {
  ...
  return (
    <p className={`mt-4 pb-2 text-6xl ${styles} ${theme.background} bg-clip-text text-transparent font-bold transition-opacity duration-300 ${isVisible ? `opacity-100` : `opacity-0`}`}>{adjectives[adjectiveNum]}</p>
  )
}
